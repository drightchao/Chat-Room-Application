...
function bindSocketEvents() {
    ...
    socket.on('presence', function(data){
            if(data.state == 'online'){
                    addClient(data.client, true, false, data.creator);
    ...
    socket.on("pvtmsgres", function(data){
	if (!data.flag) {
    ...
    socket.on("pvtmsgconf", function(data){
	if(data.agree){
...
function  roomChatDom() {
    ...
    delete_dom(document.getElementById("inside_room"));
...
function addClient(client, announce, isMe, creatorOfTheRoom){
    // show client list
    ...
    spanDom.setAttribute("id",client.nickname);
	...
	banBtnDom.addEventListener("click", function(){
	    socket.emit("unsubscribe", {currentSubscribed: currentRoom, clientKicked:client.nickname }); // unsubscribe this client
	    socket.emit("banReq",{clientName: client.nickname, banRoomName: currentRoom, creator: creator_room});
	}, false);
	...
	kickBtnDom.addEventListener("click", function(){
	    socket.emit("unsubscribe", {currentSubscribed: currentRoom, clientKicked:client.nickname}); // unsubscribe this client
	});
...
function removeClient(client, announce){
	...
	var clientDom = document.getElementById(client.nickname);
	delete_dom(clientDom);
...
function pvtmsgreqcfmyes(){
    var friend = document.getElementById("pvtmsgreqcfmfn").innerHTML;
...
function resetPvtChat(){
	...
	var friend_input_text = document.getElementById('friend_input');
	var friend_input_btn = document.getElementById('friend_button');
	friend_input_text.value = '';
	friend_input_text.disabled = false;
	friend_input_btn.disabled = false;
}
...
$("#pvt_message_input").keypress(function(event){
    if(event.keyCode == 13){
        event.preventDefault();
        $("#sendPvtMsgBtn").click();
    }
